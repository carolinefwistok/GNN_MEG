...
def get_scores(model: torch.nn.Module, x: torch.tensor, edge_index: torch.tensor, edge_attr: torch.tensor, batch: torch.tensor,
               temperature: float, train: bool) -> torch.tensor:
    device = get_device()
    model.to(device)

    if train:
        scores = model(x.to(device), edge_index.to(device), edge_attr.to(device), batch.to(device), temperature.to(device)).detach().cpu()
    else:
        with torch.no_grad():
            scores = model(x.to(device), edge_index.to(device), edge_attr.to(device), batch.to(device), temperature.to(device)).detach().cpu()
    return scores

def get_predicted_class(model: torch.nn.Module, x: torch.tensor, edge_index: torch.tensor, edge_attr: torch.tensor,
                        batch: torch.tensor, temperature: float, single_output: bool) -> torch.tensor:
    scores = get_scores(model, x, edge_index, edge_attr, batch, temperature, train=False)
...
def fidelity(graph: Data, node_set: Set[int], model: torch.nn.Module, task: Task = Task.GRAPH_CLASSIFICATION,
    ...
    if task == Task.GRAPH_CLASSIFICATION or task == Task.NODE_CLASSIFICATION:
        scores = get_scores(model, graph.x, graph.edge_index, graph.edge_attr, batch, temperature=2, train=False)
        scores_occluded = get_scores(model, x_occluded, graph.edge_index, graph.edge_attr, batch, temperature=2, train=False)
...
def _aggregate_scores(loader, model, class_idx, task: Task = Task.GRAPH_CLASSIFICATION,
    ...
    for data in iter(loader):
        if task == Task.GRAPH_CLASSIFICATION:
            scores = model(data.x.to(device), data.edge_index.to(device), data.edge_attr.to(device), data.batch.to(device)).detach().cpu()
...
def mc_l_shapley(model: torch.nn.Module, graph: Data, subgraph: Set[int], t: int, num_layers: int,
    ...
    if task == Task.GRAPH_CLASSIFICATION or task == Task.NODE_CLASSIFICATION:
        scores = model(x=graph.x.to(device), edge_index=graph.edge_index.to(device), edge_attr=graph.edge_attr.to(device),
                       batch=torch.zeros(graph.num_nodes).long().to(device),
                       temperature=2).detach().cpu()
    else:
        x1 = torch.tensor([nodes_to_keep[0]]).long().to(device)  # node 1 of edge
        x2 = torch.tensor([nodes_to_keep[1]]).long().to(device)  # node 2 of edge
    ...
    for i in range(t):
        ...
        include_data = Data(masked_x_include.float(), graph.edge_index, graph.edge_attr)
        ...
        exclude_data = Data(masked_x_exclude.float(), graph.edge_index, graph.edge_attr)
...
def fidelity_wrapper(model: torch.nn.Module, graph: Data, subgraph: Set[int], t: int, num_layers: int,
    ...
    return fidelity(graph=graph, node_set=subgraph, model=model, task=task, nodes_to_keep=nodes_to_keep)
...
def mcts_gnn_score(model: torch.nn.Module, graph: Data, subgraph: Set[int], t: int, num_layers: int,
    ...
    if task == Task.GRAPH_CLASSIFICATION or task == Task.NODE_CLASSIFICATION:
        scores = get_scores(model, graph.x, graph.edge_index, graph.edge_attr, batch, temperature=2, train=False)
        scores_subgraph = get_scores(model, x_subgraph, graph.edge_index, graph.edge_attr, batch, temperature=2, train=False)
    else:  # link prediction
        x1 = torch.tensor([nodes_to_keep[0]]).to(device)
        x2 = torch.tensor([nodes_to_keep[0]]).to(device)
        scores = model(graph.x.to(device), graph.edge_index.to(device), x1, x2, ptr=None).detach().cpu()
