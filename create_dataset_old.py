...
class MEGGraphs(Dataset):
    ...
    def __init__(self, input_type, root, filenames, stim_info_dict, duration, overlap, conn_method, fmin, fmax, ramp_time, conn_save_dir, freq_res=1, scout_data_list=None, processed_dir=None):
        ...
        INPUTS: 
            ...
            - conn_method       : string indicating the method for computing the connectivity metric
            - fmin              : minimum frequency for PSD and connectivity calculation
            - fmax              : maximum frequency for PSD and connectivity calculation
        ...
        self.conn_save_dir = conn_save_dir
    ...
    def extract_patient_and_stim(self, filename):
        '''
        Extracts the patient code and stimulation type from the filename.
        INPUT:
            - filename :        Name of the file from which to extract the patient code and stimulation type
        OUTPUT:
            - patient_code :    Patient code (PTxx/PTNxx) extracted from the filename
            - stim_type :       Stimulation type (TONIC or BURST) extracted from the filename
        '''
        # Find the patient code and stimulation type from the filename using regex
        match = re.match(r'(PTN?\d+)_([A-Z]+)_', filename)
        if match:
            patient_code = match.group(1)  # PT01 or PTN04
            stim_type = match.group(2)     # BURST or TONIC
            print(f"Extracted patient code: {patient_code}, stimulation type: {stim_type} from filename: {filename}")
    ...
    def load_existing_graphs(self):
        ...
        for idx_files in range(len(self.filenames)):
            ...
            for idx_epoch in range(self.epochs_per_file[idx_files]):
                ...
                # Set patient file and ID attributes for the graph
    ...
    def _get_nodes(self, epoch_data, sfreq, fmin, fmax, freq_res=1):
        '''
        ...
        INPUTS: 
            ...
            - freq_res      : The defined frequency resolution for the PSD calculation
    ...
    def _get_edges(self, epoch_data, sfreq, method, fmin, fmax, idx_file, idx_epoch, conn_save_dir, freq_res=1): 
        ...
        conn = mne_connectivity.spectral_connectivity_time(
            epoch_data,
            freqs=np.arange(fmin, fmax + freq_res, freq_res),  # Set frequency range, frequency resolution of 1 Hz
            method=method,
            sfreq=sfreq,
            fmin=fmin,
            fmax=fmax,
            faverage=True,
            verbose=False,
            # n_cycles=3  # Reduce number of cycles for subepoch lengths smaller than wavelet length (= n_cycles/frequency)
        )
    ...
    def remove_bad_graphs(self):
        '''
        Moves all graph files to an "all_graphs" folder, then moves bad graph files to a separate folder for bad subepochs,
